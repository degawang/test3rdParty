...
int main() 
{
	...
	try {		
		cv::Mat image;
		GLuint tex_id;
		std::string videoName("E://image//000.mp4");
		VideoCapture capture; capture.open(videoName);
		//capture.open("http://www.laganiere.name/bike.avi");
		if (!capture.isOpened()) { throw cDegaException("can not open video"); }

		int frameRate = capture.get(CV_CAP_PROP_FPS);  //帧率 x frames/s
		int totalFrames = capture.get(CV_CAP_PROP_FRAME_COUNT); //总帧数
		int frameWidth = capture.get(CV_CAP_PROP_FRAME_WIDTH);  //帧宽度
		int frameHeight = capture.get(CV_CAP_PROP_FRAME_HEIGHT); //帧高度
		...
		while (!glfwWindowShouldClose(renderWindow)) {
			...
			capture >> image;
			if (image.empty()) { for (int i = 0; i < 100; i++) { cv::waitKey(6000); } }
			...
			if (ImGui::Begin("helpWindow", &isShowHelpWindow, (helpWindowCorner != -1 ? ImGuiWindowFlags_NoMove : 0) | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoNav)) {
				ImGui::Text("s or S save current image.\n" "space compare the ori and dst image.");
				ImGui::Separator();
